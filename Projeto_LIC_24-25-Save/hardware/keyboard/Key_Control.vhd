...
process (CurrentState, Kack, Kpress)
	begin
		case CurrentState is
			...
			when STATE_PRESSING_KEY  =>   if (Kack = '1') then
														NextState <= STATE_WAITTING_RELEASE;
												   else 
														NextState <= STATE_PRESSING_KEY;
												      end if;
														
			when STATE_WAITTING_RELEASE => if (Kack = '0' and Kpress = '0') then
														NextState <= STATE_SCANNING;
													else
														NextState <= STATE_WAITTING_RELEASE;
													end if;
